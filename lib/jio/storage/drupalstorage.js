    ...
    */
      ...
      return new Promise(function (resolve) {
        if (priv.privateserver && priv.token === '') {
          ...
          if (priv.username && priv.password) {
            ...
            xhr.open('POST',
                     priv.server + '/' + priv.endpoint + '/user/login.json',
                     true);
          ...
          if (this.readyState === 4) {
            ...
            case 'that.post:initial post to get file id':
              if (this.status === 200) {
                ...
                settings.data.fid = (parseInt(settings.fid, 10) + 1).toString();
                resolve(settings);
            ...
            case 'that.post:second post with updated metadata document':
              console.log(settings.data.fid);
              console.log(JSON.parse(this.response).fid);
    ...
    function getMethodWrapper(o) {
      return new Promise(function (resolve) {
        ...
        switch (curObj.state) {
        case 'that.alldocs : get json from Drupal':
          xhr.open('GET', priv.server + '/' + priv.endpoint + '/file', true);
          xhr.setRequestHeader('Accept', 'application/json');
          break;
        default:
    ...
    function deleteMethodWrapper(o) {
      return new Promise(function (resolve) {
        ...
        xhr.open('DELETE',
                 priv.server + '/?q=' + priv.endpoint + '/file/' + o.fid,
                 true);
